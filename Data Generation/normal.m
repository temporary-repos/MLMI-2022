...
tmp_path = 'Pacing/Normal/TMP/';
bsp_path = 'Pacing/Normal/BSP/';
% get H
H = load('H.mat')
H = H.H;
...
start_xs = fix((b-a).*rand(2000,1));
start_ys = fix((b-a).*rand(2000,1));
...
pace_xs = fix((b2-a2).*rand(2000,1));
pace_ys = fix((b2-a2).*rand(2000,1));
...
%         create the extra foci dynamics if 1000 iterations are complete
        if (it == start_pace_time && iteration>1000)
            for ix = 1:Nx
                for iy = 1:Ny
...
%     mea is used to generate body surface potentials
    mea = H * tmp;
...
%         add noise to body surface potentials
    ...
    tmp_name = strcat('pace_tmp_', 'pace1_' ,string(start_x), '_', string(start_y), '_pace2_' ,string(pace_x), '_', string(pace_y), '_time_', string(start_pace_time), '.mat');
